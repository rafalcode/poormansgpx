/* from gl0
 * stragrab1.c, grab the first bits of data from a Strava file
 * this version gets out quickly so a whole pile can be done quickly */
#define  _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char *argv[])
{
    ...
    int lo0, lo1, llidx; // long comes first in strava gpx
    int la0, la1;
    ...
    while ((nread = getline(aol+asz, &len, stream)) != -1) {
        ...
        if((aol[asz][j] == 't') && (aol[asz][j+1]== 'r') && (aol[asz][j+2] == 'k') && (aol[asz][j+3]== 'p') ) {
            llidx=asz;
            while(aol[asz][j] != '"')
                j++; 
            j++;
